...
def dataset():
    ...
    Label(data_screen, text="").pack()
    Label(data_screen, text="Batch-sec * ").pack()
    class_login_entry = Entry(data_screen, textvariable=class_verify)
    class_login_entry.pack()
    Label(data_screen, text="").pack()
...
def taking_dataset():
    ...
    class1=class_verify.get()
    file = open("face.txt", "a")
    file.writelines(rollno1+"."+username1 +"."+class1+ "\n")
    file.close()
    ...
    while (True):
        ...
        for (x, y, w, h) in faces:  # corner points x y and width and height of detected face
            ...
            cv2.imwrite("dataset/User." + str(rollno1) +'.'+ str(username1) + '.' + str(class1)+'.'+str(count) + ".jpg", gray[y:y + h, x:x + w])
...
def dataset1():
    ...
    class_login_entry.pack()
    Label(data_screen, text="").pack()
    Button(data_screen, text="OK", width=10, height=1, command= taking_dataset1 or data_screen.destroy).pack()
        ...
        for (x, y, w, h) in faces:
            ...
            if (confidence < 60):
                ...
                for j in fileOpen.readlines():
                    ...
                    if (int(individual[0]) == ids):
                        id = str(individual[1])
                        classes=str(individual[2])
                confidence = "  {0}%".format(round(100 - confidence))
                xlwrite.output('attendance','class1',ids,classes[:-1],id,'yes');
...
def see_attendance():
    ...
    class_screen.title("Batch")

    class_screen.geometry("300x300")
    global class_verif
    class_verif=StringVar()
    global class_login
    Label(class_screen, text="batch-section * ").pack()
    class_login = Entry(class_screen, textvariable=class_verif)
    class_login.pack()

    Label(class_screen, text="").pack()

    Button(class_screen, text="OK", width=10, height=1, command=startAttendace or   class_screen.destroy).pack()
...
def startAttendace():
    Section = class_verif.get()
    os.startfile(os.getcwd() + "/firebase/attendance_files/" + str(Section) + "attendance" + str(datetime.now().date()) + '.xls')
def xyz():
...
def main_account_screen():
    ...
    Label(text="").pack()
    Button(text="Modification of data", height="2", bg="green", width="30", command=dataset1).pack()
    Label(text="").pack()
